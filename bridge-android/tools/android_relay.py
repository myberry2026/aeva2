...
async def _on_phone_message(state: _RelayState, raw: str) -> None:
    ...
    if not request_id:
        ...
        if msg_type == "event":
            ...
            # Special handling for voice to keep terminal clean
            if event_name in ["voice_partial", "voice_final"]:
                logger.info("VOICE: %s", event_data.get("text", ""))
            else:
                logger.info("EVENT: %s data=%s", event_name, json.dumps(event_data))
        return
