...
def android_voice_start() -> str:
    """
    Start real-time voice recognition on the phone.
    Results will appear on the phone's overlay and be sent as events to the relay.
    """
    try:
        data = _post("/voice_start", {})
        return json.dumps(data)
    except Exception as e:
        return json.dumps({"error": str(e)})
...
def android_voice_stop() -> str:
    ...
    try:
        data = _post("/voice_stop", {})
        return json.dumps(data)
    except Exception as e:
        return json.dumps({"error": str(e)})


def android_set_overlay_text(text: Optional[str]) -> str:
    """
    Set a text message to be displayed on the phone's status overlay.
    Use None or an empty string to hide the message bubble.
    """
    try:
        data = _post("/overlay", {"text": text})
        return json.dumps(data)
    except Exception as e:
        return json.dumps({"error": str(e)})
...
def android_open_app(package: str) -> str:
    ...
    """
    try:
        data = _post("/open_app", {"packageName": package})
        return json.dumps(data)
    except Exception as e:
        return json.dumps({"error": str(e)})
