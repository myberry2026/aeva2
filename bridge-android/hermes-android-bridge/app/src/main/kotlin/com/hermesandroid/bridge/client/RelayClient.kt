...
object RelayClient {
    ...
    fun sendEvent(type: String, data: Map<String, Any?>) {
        ...
        scope?.launch {
            try {
                ...
                Log.e(TAG, "Failed to send event $type: ${e.message}")
            }
        }
    }
    ...
    private fun buildWsUrl(serverUrl: String, pairingCode: String): String {
        ...
        // Remove existing protocol prefix to parse host/port correctly
        val withoutProtocol = trimmed.replace(Regex("^(https?://|wss?://)"), "")
        
        // Extract host and potential port
        val hostPart = withoutProtocol.substringBefore('/')
        
        // Default logic: If connecting to local bridge (e.g. 127.0.0.1:8765),
        // we assume the relay server is at the same host, but port 8767
        val isLocal = hostPart.contains("localhost") || hostPart.contains("127.0.0.1")
        
        val wsHost = if (isLocal) {
            val hostIp = hostPart.substringBefore(':')
            "$hostIp:8767"
        } else {
            // For remote relays, just use the provided URL and change http to ws
            hostPart 
        }

        val protocol = if (useTls) "wss" else "ws"
        return "$protocol://$wsHost/ws?token=$pairingCode"
    }
