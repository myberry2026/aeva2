...
def summarize_storyline_max():
    ...
    print(f"[*] Preparing {len(imgs)} images for UNRESTRICTED storyline analysis...")
...
Provide a cohesive narrative of the events."""
    ...
    payload = {
        ...
        "max_tokens": 8192, # 拉满到 8192
        "temperature": 0.2,
        "extra_body": {
            "num_ctx": 16384 # 尝试显式指定上下文大小
        }
    }

    with open(f"{LOG_DIR}/storyline_request_max.json", "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2, ensure_ascii=False)
    
    print(f"[*] Sending storyline request (20 images) to {MODEL} with max_tokens=8192...")
    start = time.time()
    ...
    if resp.status_code == 200:
        res_data = resp.json()
        with open(f"{LOG_DIR}/storyline_response_max.json", "w", encoding="utf-8") as f:
            json.dump(res_data, f, indent=2, ensure_ascii=False)
        
        msg = res_data['choices'][0]['message']
        print(f"\n[*] SUCCESS! Latency: {dt:.2f}s")
        print("\n" + "="*50 + " STORYLINE SUMMARY (UNRESTRICTED) " + "="*50)
        print(f"\n[REPLY]\n{msg.get('content', '')}")
        print("="*119)
    else:
