...
def test_inference_thinking():
    filename = "test_inf_thinking.wav"
    generate_tone(filename, duration_sec=0.1, freq=2000.0)
    audio_b64 = encode_file(filename)
    
    print(f"--- Inference + Thinking Test for {MODEL} ---")
    ...
    try:
        ...
        res = r.json()
        print(f"Thinking: {res['choices'][0]['message'].get('reasoning_content')}")
        print(f"Response: {res['choices'][0]['message'].get('content')}")
    finally:
        if os.path.exists(filename):
            os.remove(filename)

if __name__ == "__main__":
    test_inference_thinking()
